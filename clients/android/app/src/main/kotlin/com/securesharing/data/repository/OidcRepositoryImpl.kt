...
import com.securesharing.data.remote.dto.AuthProviderDto
import com.securesharing.data.remote.dto.OidcAuthorizeRequest
import com.securesharing.data.remote.dto.OidcCallbackRequest
import com.securesharing.data.remote.dto.OidcRegisterRequest
import com.securesharing.domain.model.AuthProvider
import com.securesharing.domain.model.User
import com.securesharing.domain.repository.OidcAuthorizeResult
import com.securesharing.domain.repository.OidcCallbackResult
...
class OidcRepositoryImpl @Inject constructor(
    ...
    override suspend fun beginAuthorize(providerId: String): Result<OidcAuthorizeResult> {
        return try {
            val request = OidcAuthorizeRequest(
                ...
                redirectUri = "securesharing://oidc/callback"
            )
            ...
            when (body.status) {
                "authenticated" -> {
                    ...
                    val vaultKey = cryptoManager.hkdfDerive(
                        keyMaterial, keySalt, "securesharing-vault-key".toByteArray()
                    )
    ...
    override suspend fun completeRegistration(
        ...
        return try {
            ...
            val vaultKey = cryptoManager.hkdfDerive(
                keyMaterialBytes, keySaltBytes, "securesharing-vault-key".toByteArray()
            )
