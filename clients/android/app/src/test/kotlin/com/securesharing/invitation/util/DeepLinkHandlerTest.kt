package com.securesharing.invitation.util
 ...
 *
 * Tests cover:
 * - Custom scheme parsing (securesharing://)
...
class DeepLinkHandlerTest {
    ...
    fun `parseIntent with http scheme invite returns AcceptInvitation action`() {
        val intent = createMockViewIntent(
            scheme = "https",
            host = "app.securesharing.example",
    ...
    fun `generateShareLink creates correct URI`() {
        ...
        every { expectedUri.toString() } returns "securesharing://share/share123"
        every { Uri.parse("securesharing://share/share123") } returns expectedUri

        val uri = deepLinkHandler.generateShareLink("share123")

        assertEquals("securesharing://share/share123", uri.toString())
    }

    @Test
    fun `generateWebShareLink creates correct URI with default base URL`() {
        val expectedUri = mockk<Uri>()
        every { expectedUri.toString() } returns "https://securesharing.example.com/share/share123"
        every { Uri.parse("https://securesharing.example.com/share/share123") } returns expectedUri

        val uri = deepLinkHandler.generateWebShareLink("share123")

        assertEquals("https://securesharing.example.com/share/share123", uri.toString())
    }
