...
class CryptoManagerTest {
    ...
    // Test data
    private val testPlaintext = "Hello, SecureSharing!".toByteArray()
    ...
    fun `deriveKeyLegacy calls hkdfProvider with correct parameters`() {
        ...
        every {
            hkdfProvider.deriveKey(testPassword, testSalt, info = "SecureSharing-v1".toByteArray())
        } returns derivedKey

        val result = cryptoManager.deriveKeyLegacy(testPassword, testSalt)

        verify { hkdfProvider.deriveKey(testPassword, testSalt, info = "SecureSharing-v1".toByteArray()) }
    ...
    fun `deriveKey uses Argon2id output with HKDF`() {
        ...
        every {
            hkdfProvider.deriveKey(
                ikm = any(),
                salt = "SecureSharing-MasterKey-v1".toByteArray(),
