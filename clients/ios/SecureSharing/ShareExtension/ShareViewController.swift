...
/// Share Extension for SecureSharing
/// Allows users to share files from other apps directly into SecureSharing
class ShareViewController: UIViewController {
    ...
    private let titleLabel: UILabel = {
        let label = UILabel()
        label.text = "Share to SecureSharing"
    ...
    @objc private func uploadTapped() {
        ...
        saveFilesToAppGroup { [weak self] success in
            DispatchQueue.main.async {
                if success {
                    self?.statusLabel.text = "Opening SecureSharing..."
                    self?.openMainApp()
                } else {
    ...
    private func saveFilesToAppGroup(completion: @escaping (Bool) -> Void) {
        guard let containerURL = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: "group.com.securesharing"
        ) else {
            completion(false)
            return
        }
    ...
    private func openMainApp() {
        ...
        guard let url = URL(string: "securesharing://import") else {
            completeRequest()
            return
        }
