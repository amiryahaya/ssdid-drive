...
This test plan covers the invitation flow in the iOS SsdidDrive app, including:
...
| Test Case | Description | Expected Result |
|-----------|-------------|-----------------|
| `test_customScheme_parsesInviteToken` | `ssdid-drive://invite/{token}` | Extracts token correctly |
| `test_universalLink_parsesInviteToken` | `https://app.ssdid-drive.com/invite/{token}` | Extracts token correctly |
    ...
    - name: Run Invitation Tests
      run: |
        xcodebuild test \
          ...
          -only-testing:SsdidDriveTests/InvitationTests \
          -only-testing:SsdidDriveTests/InviteAcceptViewModelTests \
          -only-testing:SsdidDriveTests/AuthRepositoryTests \
