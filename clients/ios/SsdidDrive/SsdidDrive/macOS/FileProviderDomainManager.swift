...
/// Manages the NSFileProviderDomain lifecycle for SsdidDrive.
...
final class FileProviderDomainManager {
    ...
    private let domainIdentifier = NSFileProviderDomainIdentifier(rawValue: "my.ssdid.drive.user-files")
    #endif

    /// Register the File Provider domain after successful login.
    func registerDomain(displayName: String = "SsdidDrive") {
        #if !targetEnvironment(macCatalyst)
    ...
    func unregisterDomain() {
        #if !targetEnvironment(macCatalyst)
        let domain = NSFileProviderDomain(identifier: domainIdentifier, displayName: "SsdidDrive")
