...
/// Main File Provider extension class that integrates SSDID Drive with Finder
class FileProviderExtension: NSObject, NSFileProviderReplicatedExtension {
    ...
    private let logger = Logger(subsystem: "my.ssdid.drive.fileprovider", category: "Extension")
    ...
    private func requestDecryption(encryptedURL: URL, fileId: String) async throws -> URL {
        ...
        DistributedNotificationCenter.default().postNotificationName(
            NSNotification.Name("my.ssdid.drive.cryptoRequest"),
            object: nil,
        ...
        while Date().timeIntervalSince(startTime) < timeout {
            if let response = sharedDefaults.readCryptoResponse(requestId: requestId) {
                ...
                if let error = response.error {
                    throw NSError(domain: "my.ssdid.drive.crypto", code: -1, userInfo: [NSLocalizedDescriptionKey: error])
                }
