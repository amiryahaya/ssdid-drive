...
This document describes the threat model for SSDID Drive, identifying potential threats, attack vectors, and mitigations implemented in the system.
...
│  │  • SSDID Wallet (DID-based challenge-response)                     │    │
│  │  • Wallet is trusted for authentication, NOT for key material       │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                              ▲                                              │
│                              │ Trust Boundary                               │
│                              ▼                                              │
...
|--------|--------|------------|------------|
| Credential theft | High | Medium | DID key binding (hardware-backed) |
| Session hijacking | High | Low | Bearer tokens, short expiry (Redis-backed) |
| Phishing | High | Medium | DID challenge-response verification |
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  User loses device with wallet credentials                                  │
│                                                                              │
│  If attacker has device:                                                    │
│  • Cannot use wallet without biometric/PIN                                 │
│  • DID private key requires device unlock                                  │
...
│  ✓ Old wallet credentials invalidated                                      │
│                                                                              │
...
| Credential verification | Detect credential reuse | DID challenge nonces |
...
- HKDF-SHA3-256 is a secure key derivation function

### 7.3 SSDID Wallet

- Wallet correctly manages DID key pairs
- Wallet protects private keys (hardware-backed where available)
- Challenge-response protocol is implemented correctly
